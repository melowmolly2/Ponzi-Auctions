...
Dự án là một hệ thống đấu giá trực tuyến hoạt động theo mô hình Client-Server, cho phép nhiều người dùng cùng tham gia cạnh tranh giá để mua sản phẩm trong một khoảng thời gian xác định. 
Hệ thống phân quyền người dùng với 3 vai trò chính:
*   **Admin:** Quản lý toàn bộ hệ thống, kiểm duyệt và cấm/mở khóa người dùng.
*   **Seller:** Đăng bán vật phẩm, thiết lập giá khởi điểm và thời gian phiên đấu giá.
*   **Bidder:** Tham gia đặt giá (Bid) cạnh tranh, hỗ trợ các tính năng như đấu giá tự động (Auto-bidding).
...
**Phần Backend (`/src/main/java/com/auction`)**:
...
*   **Real-time Sinks:** `ItemPricesSink.java` và `UserBalanceSink.java` phát luồng dữ liệu thời gian thực (SSE).

**Phần Frontend (`/src/main/java/` và `/src/main/resources/`)**:
*   `controller`: Chứa các controller UI của JavaFX (`AdminPage`, `BidderViewPage`, `SellerViewPage`,...).
*   `network`: Xử lý kết nối, giao tiếp qua API RESTful (`ApiClient`) và duy trì luồng dữ liệu (`PriceStreamListener`, `BalanceStreamListener`).
...
**Lưu ý quan trọng:** Cần phải khởi chạy máy chủ (Server) trước, sau đó mới khởi chạy máy khách (Client).
...
*   ✅ **Chức năng Bắt buộc:**
    *   Quản lý đăng ký / đăng nhập (JWT), cấp quyền Admin/Seller/Bidder.
    *   Quản lý sản phẩm đấu giá (Thêm, sửa, xóa, thiết lập thời gian).
    *   Tự động khóa phiên đấu giá khi hết giờ, xác định người thắng.
    *   Xử lý các ngoại lệ (lỗi kết nối, đặt giá thấp hơn giá hiện hành).
    *   Giao diện đồ họa (GUI) đầy đủ bằng JavaFX cho từng loại người dùng.
*   🔥 **Chức năng Kỹ thuật & Nâng cao:**
    *   **Xử lý Đấu giá Đồng thời (Concurrent Bidding):** Tận dụng luồng của Spring Boot và Database Lock để giải quyết bài toán tranh chấp (ngăn chặn lost update, loại bỏ race condition).
    *   **Cập nhật Thời gian thực (Realtime Update):** Sử dụng SSE (Server-Sent Events) kết hợp thiết kế Observer Pattern nâng cao để tự động đẩy luồng giá/số dư trực tiếp đến Client mà không sử dụng polling gây quá tải.
    *   **Auto-Bidding:** Hỗ trợ thiết lập giá thầu tối đa (`maxBid`) và bước giá (`increment`), hệ thống tự động tranh giá thay mặt người dùng.
    *   **Lịch sử Giá (Bid History):** Ghi nhận và lưu lại luồng thời gian thực của các lượt bid.
